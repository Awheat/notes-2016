$(function(){
    ...
    错误提示框
    */
    var alertBox = function(_title,_cont){
    ...
    */
    ...
    判断标签是否必填
    */
    var requiredJudge = function(_value,_element){
    ...
    */
    $.validator.addMethod("isName",function(_value,_element) {
        console.log($(_element).attr("aria-required"));
        var regexp = new RegExp("^[\u4e00-\u9fa5]{2,5}$");
        _element.value = trimCenter(_value);
        return regexp.test(trimCenter(_value));
    }, "请输入2至5位中文字符");
    ...
    */
    $.validator.addMethod("isCreditCode",function(_value,_element) { 
        ...
        console.log(requiredJudge(_value,_element));
        if(requiredJudge(_value,_element)) return true;
        var selectedVal = $("#credit_type").find('option:selected').html();
        console.log("证件类型："+selectedVal);
        if("身份证" == selectedVal){
    ...
    */
    var creditCode = {
        "creditIdCode":function(_value){
            ...
            var idCard = trimCenter(_value); //去掉字符串头尾空格   
            if(idCard.length != 18){
                return false;
            }                  
